import curses
import time

# Initialize global variables
phone_connected = False
dispenser_connected = False
task_signal = 1

def draw_menu(stdscr):
    global phone_connected, dispenser_connected, task_signal
    k = 0

    # Start colors in curses
    curses.start_color()
    curses.init_pair(1, curses.COLOR_CYAN, curses.COLOR_BLACK)

    # Loop where k is the last character pressed
    while k != ord('q'):  # Press 'q' to exit
        # Initialization
        stdscr.clear()
        height, width = stdscr.getmaxyx()

        # Handle input
        if k == ord('p'):
            phone_connected = not phone_connected
        elif k == ord('d'):
            dispenser_connected = not dispenser_connected
        elif ord('0') <= k <= ord('4'):
            task_signal = k - ord('0')

        # Render the UI
        stdscr.addstr(1, 1, '****************************************************')
        stdscr.addstr(2, 1, '*                                                  *')
        stdscr.addstr(3, 1, '*    Phone connected: [{:<5}]                      *'.format(str(phone_connected)))
        stdscr.addstr(4, 1, '*    Dispenser connected: [{:<5}]                  *'.format(str(dispenser_connected)))
        stdscr.addstr(5, 1, '*    Task signal: [{:<2}]                             *'.format(task_signal))
        stdscr.addstr(6, 1, '*                                                  *')
        stdscr.addstr(7, 1, '*    Press "p" to toggle phone connection          *')
        stdscr.addstr(8, 1, '*    Press "d" to toggle dispenser connection      *')
        stdscr.addstr(9, 1, '*    Press numbers (0-4) to change task signal     *')
        stdscr.addstr(10, 1, '*    Press "q" to quit                             *')
        stdscr.addstr(11, 1, '*                                                  *')
        stdscr.addstr(12, 1, '****************************************************')

        # Refresh the screen
        stdscr.refresh()

        # Run control loop if phone is connected
        if phone_connected:
            perform_action(stdscr, height, width)

        # Wait for next input
        k = stdscr.getch()

def perform_action(stdscr, height, width):
    global task_signal, dispenser_connected

    if dispenser_connected:
        # Set the strength with the task_signal variable
        strength = task_signal

        # Wait for next input
        k = stdscr.getch()

        # If 'p' is pressed, send a "pulse" of a task completed
        if k == ord('p'):
            # Send a "pulse" of a task completed
            stdscr.addstr(height - 2, 2, 'Dispensing at strength: {} '.format(strength) + " " * (width - len('Dispensing at strength: {} '.format(strength)) - 3))
            stdscr.refresh()

            # Make the pulse appear on the UI for only a second
            stdscr.timeout(1000)  # Set the timeout to 1 second

            # Wait for next input
            k = stdscr.getch()

            # If no input was received within the timeout, clear the pulse message
            if k == -1:
                stdscr.addstr(height - 2, 2, " " * (width - 3))
                stdscr.refresh()

            # Reset the timeout
            stdscr.timeout(-1)

def main():
    curses.wrapper(draw_menu)

if __name__ == "__main__":
    main()